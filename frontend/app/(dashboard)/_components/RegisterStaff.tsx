...
import { loadUserFromStorage, registerStaff } from "@/store/slices/authSlice";
...
const RegisterStaff = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const registerPromise = dispatch(registerStaff(finalFormData)).unwrap(); // 👈 call context function

    try {
      await toast.promise(registerPromise, {
        loading: "creating staff account...",
        success: "successfully created staff account",
        error: <b>{error}</b>,
      });
  ...
  return (
    <div className="flex flex-col md:flex-row items-center justify-center p-5 relative bg-no-repeat bg-cover min-h-screen px-4 md:px-10 lg:px-20">
      <div className="w-full p-6">
        <div className="mb-8">
          <h1 className="text-xl font-medium">Create a staff account</h1>
          <p className="text-[11px] text-muted-foreground">
            Enter the details below to create a staff account
          </p>
          {/* logo goes here */}
        </div>
